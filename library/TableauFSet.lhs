...
> data PNP a
>   = PNP {typ :: PNPType, pos :: FSet a, neg :: FSet a }
>   deriving (Eq, Ord)
...
> closed p = (F `FSet.member` pos p)
>   || not (FSet.null (pos p `FSet.intersection` neg p))
...
> buildRoot f = PNP Norm (FSet.fromList [f, ever end]) FSet.empty
...
The `makeSucc` function is defined as a series of case analyses:
  ...
  6. Checking for the end of time (`FSet.member (X top) (neg q)`)
...
>   if closed q  -- Bot
>   then []
>   else case FSet.pickImp (pos q) of -- (->+)
>     -- Just (Imp F _, posq') -> [q {pos = posq'}] -- opt for forms equiv to TOP
>     Just ((a,b), posq') -> posImp (typ q) a b posq' (neg q)
>     Nothing ->
>       case FSet.pickImp (neg q) of -- ( -> -)
>         Just ((a,b), negq') -> negImp (typ q) a b (pos q) negq'
>         Nothing ->
>           case FSet.pickW (pos q) of -- (W+)
>             Just ((a,b), posq') -> posW (typ q) a b posq' (neg q)
>             Nothing ->
>               case FSet.pickW (neg q) of -- (W-)
>                 Just ((a,b), negq') -> negW (typ q) a b (pos q) negq'
>                 Nothing ->
>                   if (X top `FSet.member` neg q) --  && null posq'
...
using `pickImp` to remove `Imp a b` from the `posq'`.

> posImp :: Ord a => PNPType -> LTL a -> LTL a -> FSet a -> FSet a -> [PNP a]
> posImp t a b posq' negq = let t' = stepType t in
>   [
>     PNP t' (FSet.insert b $ posq') negq 
>   , PNP t' posq' (FSet.insert a $ negq)
>   ]
...
> negImp :: Ord a => PNPType -> LTL a -> LTL a -> FSet a -> FSet a -> [PNP a]
> negImp t a b posq negq' =
>   [
>      PNP (stepType t) (FSet.insert a posq) (FSet.insert b negq')
>   ]
...
> posW :: Ord a => PNPType -> LTL a -> LTL a -> FSet a -> FSet a -> [PNP a]
> posW t a b posq' negq = let t' = stepType t in
>   [
>     PNP t' (FSet.insert b posq') negq
>   , PNP t' (FSet.insert a $ FSet.insert (wkX (a `W` b)) $ posq') negq
>   ]
...
> negW :: Ord a => PNPType -> LTL a -> LTL a -> FSet a -> FSet a -> [PNP a]
> negW t a b posq negq' = let t' = stepType t in
>   [
>     PNP t' posq (FSet.insert a $ FSet.insert b negq')
>   , PNP t' (FSet.insert (X $ negate $ W a b) posq) (FSet.insert b negq')
>   ]
...
If we decide that we can take a step forwards, the `FSet.unfoldX` function
...
To advance time, we step both sets of a PNP---marking the resulting
PNP as having `Temp`oral type:

> stepPNP :: Ord a => PNP a -> [PNP a]
> stepPNP (PNP _ posq negq) = [PNP Temp (FSet.unfoldX posq) (FSet.unfoldX negq)]

On the other hand, if we are at the end of time, we cannot take any
steps.  Given a PNP `q`, we can generate a `terminalPNP` that restricts `q`'s
...
> terminalPNP q = [PNP Term (FSet.terminate $ pos q)
>                           (FSet.insert (X top) $ FSet.terminate $ neg q)]
